...
class StarPilotVCruise:
  ...
  def update(self, controls_enabled, now, time_validated, v_cruise, v_ego, sm, starpilot_toggles):
    ...
    if force_standstill_enabled and not self.override_force_standstill:
      ...
      # Waze Police & Hazard Auto-Slowdown (drops cruise target strictly to raw posted speed limit without offset)
      if (get_shm_param("WazePoliceSlowdownActive", False) or get_shm_param("RoadHazardSlowdownActive", False)):
